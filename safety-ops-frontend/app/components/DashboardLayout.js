...
export default function DashboardLayout({ children, title }) {
    ...
    const getHomeLink = () => {
        ...
        if (role === 'responder') return '/worker-dashboard'; // Fixed redirection for Responder
        if (role === 'worker') return '/worker-dashboard';
        return '/dashboard'; // Citizen
    };

    // ✅ YOUR COMPLETE NAVIGATION LIST
    const navItems = [
    ...
    return (
        <div className="flex min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden">
            ...
            `}>
                ...
                {/* BRANDING (Desktop) */}
                <div className="h-16 flex items-center px-6 border-b border-gray-800 bg-[#0B0C10] hidden md:flex">
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto mt-16 md:mt-0">
                    ...
                    {navItems.map((item) => {
                        if (item.roles.includes(role)) {
                            ...
                            return (
                                <Link 
                                    key={item.label} // ✅ Uses unique Label as key
                                    href={item.href}
                                    onClick={() => setIsMobileMenuOpen(false)} // ✅ Closes menu on mobile click
            ...
            {/* --- 3. MAIN CONTENT --- */}
            <main className="flex-1 relative flex flex-col min-w-0 pt-16 md:pt-0"> {/* Added top padding for mobile header */}
            ...
            {/* --- 4. OVERLAY (Mobile Background Dimmer) --- */}
            {isMobileMenuOpen && (
                <div 
